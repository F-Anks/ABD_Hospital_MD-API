...
--
-- Host: 127.0.0.1    Database: hospital_230758
-- ------------------------------------------------------
...
--
-- Dumping events for database 'hospital_230758'
--

--
-- Dumping routines for database 'hospital_230758'
--
/*!50003 DROP PROCEDURE IF EXISTS `sp_limpiar_tablas_pacientes` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_prueba1_poblar_100k` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_prueba1_poblar_100k`(IN p_cantidad INT)
BEGIN
    ...
    WHILE i < p_cantidad DO
        ...
        SET v_edad = FLOOR(RAND() * 121); -- Rango: 0 a 120 años
        ...
        ELSE
            ...
            ELSE
                SET v_nombre = ELT(FLOOR(1 + RAND() * 10), 'Alex','Ariel','Angel','Guadalupe','Cruz','Leslie','Dominique','Charlie','Dana','Robin');
            END IF;
            ...
            IF v_edad < 18 THEN
                IF v_pais_origen = 'Mexico' AND v_edad > 0 THEN
                    SET v_curp = CONCAT('CURP', LPAD(FLOOR(RAND() * 100000000000000), 14, '0'));
                END IF;
            ELSE
                IF v_pais_origen = 'Mexico' THEN
                    SET v_curp = CONCAT('CURP', LPAD(FLOOR(RAND() * 100000000000000), 14, '0'));
                    IF RAND() < 0.6 THEN
                        SET v_rfc = CONCAT('RFC', LPAD(FLOOR(RAND() * 10000000000), 10, '0'));
                    END IF;
                END IF;
    ...
    SELECT CONCAT('Se han insertado ', p_cantidad, ' pacientes (Prueba 1 exitosa).') AS Resultado;
...
/*!50001 VIEW `vw_reporte_privilegios` AS select `reporte`.`mensaje` AS `mensaje` from (select '-- GERENCIA --' AS `mensaje` union all select concat('GRANT ',group_concat(`tp`.`PRIVILEGE_TYPE` order by `tp`.`PRIVILEGE_TYPE` ASC separator ', '),' ON ',`tp`.`TABLE_SCHEMA`,'.',`tp`.`TABLE_NAME`,' TO ',`tp`.`GRANTEE`,';') AS `CONCAT(
        ...
        tp.GRANTEE, ';'
            ...
            sp.GRANTEE,
            ';'
        )` from `information_schema`.`SCHEMA_PRIVILEGES` `sp` where ((`sp`.`TABLE_SCHEMA` = 'hospital_230758') and (`sp`.`GRANTEE` like '\'developer\'@%')) group by `sp`.`GRANTEE`,`sp`.`TABLE_SCHEMA`) `reporte` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-03-13 13:40:38
